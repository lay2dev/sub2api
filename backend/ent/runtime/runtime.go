...
func init() {
	...
	redeemcodeDescMaxUses := redeemcodeFields[6].Descriptor()
	...
	redeemcodeDescUsedCount := redeemcodeFields[7].Descriptor()
	...
	redeemcodeusageDescUsedAt := redeemcodeusageFields[3].Descriptor()
	...
	// userDescBindingAddress is the schema descriptor for binding_address field.
	userDescBindingAddress := userFields[6].Descriptor()
	// user.DefaultBindingAddress holds the default value on creation for the binding_address field.
	user.DefaultBindingAddress = userDescBindingAddress.Default.(string)
	// user.BindingAddressValidator is a validator for the "binding_address" field. It is called by the builders before save.
	user.BindingAddressValidator = func() func(string) error {
		validators := userDescBindingAddress.Validators
		fns := [...]func(string) error{
			validators[0].(func(string) error),
			validators[1].(func(string) error),
		}
		return func(binding_address string) error {
			for _, fn := range fns {
				if err := fn(binding_address); err != nil {
					return err
				}
			}
			return nil
