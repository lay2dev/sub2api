...
func (s *AuthService) bindingMnemonic() string {
	if s == nil || s.cfg == nil {
		return ""
	}
	return strings.TrimSpace(s.cfg.Wallet.BindingMnemonic)
}

func (s *AuthService) maybeAssignBindingAddress(ctx context.Context, user *User) error {
	if user == nil || user.ID == 0 || user.BindingAddress != "" {
		return nil
	}

	mnemonic := s.bindingMnemonic()
	if mnemonic == "" {
		return nil
	}

	address, err := DeriveEVMBindingAddress(mnemonic, user.ID)
	if err != nil {
		return fmt.Errorf("derive binding address: %w", err)
	}
	user.BindingAddress = address
	if err := s.userRepo.Update(ctx, user); err != nil {
		return fmt.Errorf("persist binding address: %w", err)
	}
	return nil
}

func (s *AuthService) createUserWithRegistrationFinalizers(ctx context.Context, user *User, invitationRedeemCode *RedeemCode) error {
	useTx := s.entClient != nil && (invitationRedeemCode != nil || s.bindingMnemonic() != "")
	if !useTx {
		if err := s.userRepo.Create(ctx, user); err != nil {
			return err
		}
		if err := s.maybeAssignBindingAddress(ctx, user); err != nil {
			return err
		}
	...
	if err := s.maybeAssignBindingAddress(txCtx, user); err != nil {
		return err
	}
	if invitationRedeemCode != nil {
...
func (s *AuthService) RegisterWithVerification(ctx context.Context, email, password, verifyCode, promoCode, invitationCode string) (string, *User, error) {
	...
	if err := s.createUserWithRegistrationFinalizers(ctx, user, invitationRedeemCode); err != nil {
...
func (s *AuthService) LoginOrRegisterOAuth(ctx context.Context, email, username string) (string, *User, error) {
	...
	if err != nil {
		if errors.Is(err, ErrUserNotFound) {
			...
			if err := s.createUserWithRegistrationFinalizers(ctx, newUser, nil); err != nil {
				if errors.Is(err, ErrEmailExists) {
...
func (s *AuthService) LoginOrRegisterOAuthWithTokenPair(ctx context.Context, email, username, invitationCode string) (*TokenPair, *User, error) {
	...
	if err != nil {
		if errors.Is(err, ErrUserNotFound) {
			...
			if err := s.createUserWithRegistrationFinalizers(ctx, newUser, invitationRedeemCode); err != nil {
				if errors.Is(err, ErrEmailExists) {
					user, err = s.userRepo.GetByEmail(ctx, email)
					if err != nil {
						logger.LegacyPrintf("service.auth", "[Auth] Database error getting user after conflict: %v", err)
